...
1. Read `evals/extraction/dataset.v0.jsonl` (labeled cases only — skip items with `"gold": null`). A run named "baseline" requires ≥30 labelled cases (Baseline v0.1 minimum); smaller runs are calibration smoke tests and must be named as such.
...
3. Write `evals/extraction/results/<YYYY-MM-DD>-baseline/outputs.jsonl` — one line per case: `{id, gold, model_output, error?}`.
...
6. Never write anything outside `evals/extraction/results/`.
