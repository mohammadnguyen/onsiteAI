...
empty ones. A structured result that fails the plugin's own JSON Schema anywhere —
nested fields, array items, enums and bounds included. A channel whose
outcome is unknown. A history chain that cannot be followed. A plugin call
that failed. A missing or empty result. No structured result
