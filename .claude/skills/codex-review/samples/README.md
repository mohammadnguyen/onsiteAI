...
| `06-adversarial-clean.json` | Structured channel returns `approve` with an **empty** findings list | Usable, and the second of the three outcomes: *completed, reported nothing*. `findings: []` is a real answer; an absent `findings` key would not be, and is a protocol violation rather than an empty result. An approve here would still not release a blocking finding raised by the other channel. |
| `07-adversarial-inconclusive.json` | Schema-valid `needs-attention` with an **empty** findings list, the concern stated only in `summary` and `next_steps` | Usable, and **completed *with* findings** — the verdict is the finding. Recording it as "reported nothing" because the list is empty is the mistake this sample exists to catch. Disposition the concern, or go back for the specifics inside the existing budget. |
